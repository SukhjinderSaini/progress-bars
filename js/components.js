...
client.get("http://pb-api.herokuapp.com/bars",function (data) {
	    data = JSON.parse(data);
		updateButtons = data.buttons;
		barValues = data.bars;
		limit = data.limit;
		if(barValues.length>0){
			for(var barId=0;barId<barValues.length;barId++){
				if(barId==0){
				createProgressBar('progressBar'+(barId+1), barValues[barId], 'container');
				}else{
				createProgressBar('progressBar'+(barId+1), barValues[barId], 'custom');
				}
			}
			}
	});
 ...
 */
function updateProgressBar(barId, value, limit) {
